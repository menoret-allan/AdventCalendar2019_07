...
//let ``first test ampli `` (input, expected) =
//    let result = findOptimal input
...
let ``first test ampli2`` (input, expected) =
    let result = findOptimal input
    result |> should equal expected

[<Fact>]
let ``Specific test :)`` () =
    let result = findSpeOptimal "3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5" [9;8;7;6;5]
